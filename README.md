...
* **New alongside this firmware: [Mote for Android](https://github.com/austinio7116/mote/releases/latest).** A 1.3 MB app — `mote-android-arm64.apk`, on the [latest Mote release](https://github.com/austinio7116/mote/releases/latest) — that runs the whole Mote library on a phone: the launcher, the gallery, save slots, multiplayer and every game, with the buttons drawn where you can see them on a photograph of the console. It also works as a **dock**: plug your ThumbyOne into the phone with a USB-C cable and it gets internet and LAN matches from its own lobby, plus gallery installs and updates, over the phone's mobile data — no PC and no Mote Studio needed. Ships with **Mote Studio 0.21-alpha**.
